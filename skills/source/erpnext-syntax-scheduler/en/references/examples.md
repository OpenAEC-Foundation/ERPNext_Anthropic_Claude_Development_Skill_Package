...
Working examples for scheduler and background jobs.

## 1. Data Import with Progress Tracking
...
def check_pending_imports():
    """Daily check for pending imports."""
...
def process_import(import_name):
    """Process data import with progress tracking."""
...
## 3. Cleanup Job with Error Recovery
...
def deep_cleanup():
    """Weekly deep cleanup - runs on long queue."""
    doctypes_to_clean = [
        ("Error Log", 7),      # 7 days
        ("Activity Log", 30),  # 30 days
        ("Version", 90),       # 90 days
    ]
...
## 4. Report Generation with Notifications
...
def generate_report(report_type, filters):
    ...
    if is_job_enqueued(job_id):
        frappe.throw("Report is already being generated")
...
## 5. External API Sync with Rate Limiting
...
def sync_external_data():
    """Sync data from external API with rate limiting."""
...
## 6. Document Controller with queue_action
...
class SalesOrder(Document):
    ...
    def process_submission(self, notify_user):
        """Background processing after submit."""
        # Heavy operations
