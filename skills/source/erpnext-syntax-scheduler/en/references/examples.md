...
def daily_sync():
    """Synchronize data from external API daily."""
...
## 2. Email Queue with Deduplication (v15)
...
class Newsletter(Document):
    def on_submit(self):
        ...
        if is_job_enqueued(job_id):
            frappe.msgprint("Newsletter is already being sent")
            return
        ...
        frappe.msgprint("Newsletter sending started")

def notify_completion(job, connection, result, *args, **kwargs):
    frappe.publish_realtime(
        "show_alert",
        {"message": "Newsletter sent!", "indicator": "green"}
    )
...
## 4. Cron Job: Every 15 Minutes Cleanup
...
## 5. Document Processing with enqueue_doc
